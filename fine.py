...
my_parser.add_argument("--maxepoch", type=int, default=5)

args = my_parser.parse_args()

print(vars(args))
# save_path = os.path.join(args.load_path, f"fine-{args.task}-{args.ds}-{args.maxsamples}")
...
print(len(train_set), len(val_set), len(labels), flush=True)
...
for epoch in range(args.maxepoch):
    ...
    print("", flush=True)
    torch.save(model.state_dict(), f"{args.load_path}-{args.ds}-{epoch}.pt")
    write_gens(gens, f"{args.load_path}-{args.ds}-{epoch}.txt")
